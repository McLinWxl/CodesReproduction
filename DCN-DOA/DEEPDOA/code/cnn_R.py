...
S_label=read_temp['S_label']               #(19800,120)  稀疏
R_est=read_temp['R_est']                   #(19800,56)
...
cnn_tanh.compile(loss='mse', optimizer=optimizer, metrics=['accuracy'])
cnn_tanh.summary()

history_cnn_tanh=cnn_tanh.fit(S_est, S_label1,epochs=nb_epoch, batch_size=batch_size,shuffle=True
                ,verbose=2,validation_split=0.2)

val_acc = history_cnn_tanh.history['val_loss'] 
val_acc2 = history_cnn_tanh.history['loss'] 

history_cnn_tanh.history['acc']
print(history_cnn_tanh.history.keys())
...
plt.legend(['DCN+relu','DCN+tanh','DCN+sig','Method in [5]1','Method in [5]2','Method in [5]3','Method in [5]4','Method in [5]5','Method in [5]6','DNN+relu','DNN+tanh','DNN+sig'],loc='upper left', bbox_to_anchor=(1, 0.95))
