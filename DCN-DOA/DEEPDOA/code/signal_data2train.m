...
D_stop=59;
for l=1:length(k)
    DOA1=D_start:1:D_stop-k(l);   %第一个信号和第二个信号产生的方向
...
for i=1:length(DOA_train)
    [X1,~]=signal_generate(M,snapshot,DOA_train(1,i),f0,fc,fs,1);     %生成信号
    [X2,~]=signal_generate(M,snapshot,DOA_train(2,i),f0,fc,fs,1);
    temp1=awgn(X1,-10*rand,'measured');            %添加高斯白噪声
    temp2=awgn(X2,-10*rand,'measured');
    X= temp1+ temp2;
    [R_est(i,:),Rx]=feature_extract_R(X) ;
    temp=H'*vec(Rx);
    temp=temp/norm(temp);
    S_est(i,:,1)=real(temp);          %实部
    S_est(i,:,2)=imag(temp);          %虚部
    S_abs(i,:)=[real(temp);imag(temp)];%实部虚部放在一起
    S_label(i,round(DOA11(i))+61)=1;  %把[-60，60]的角度放到[0，120]的维度
